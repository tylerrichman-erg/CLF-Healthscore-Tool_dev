...
class DataService:
    ...
    @transaction.atomic
    def load_ats(self, vintage: str, state: str, county: str, tracts: list):
        """
        :param vintage: i.e. '2019'
        :param state: i.e. 'MA'
        :param county: i.e. '009'
        ...
        ats_loader = AirToxScreenDataLoader(vintage, self.user)

        for d in ats_loader.datasources.keys():
            ...
            try:
                ...
                self.log.info(f'Dataset for {datasource.name} / {vintage} does not exist. Creating...')
                dataset = Dataset.objects.create(data_source=datasource, vintage=vintage,
                                                 descriptor="AirToxScreen environmental data vintages vary. Check datasource reference URL for more information.")

            # Load the metrics so that we can perform a reverse lookup on the census code if we end up needing
            # to fetch any of the geoids from the API.
            # (i.e. 'B01003_001' -> 'Total Population')
            metrics = ats_loader.metrics[d]
            ...
            for tract in tracts:
                ...
                if not all(metric_exists.values()):
                    ...
                    try:
                        fetched_df = ats_loader.fetch_tract_data(d, state.fips_code, county, tract)
                        ...
                        print("Error fetching AirToxScreen missing metrics for tract: " + tract)
                        return all_records
        ...
        all_geoids = [state.fips_code] + geoids
        return Metric.objects.filter(
            dataset__data_source__name__in=ats_loader.datasources.keys(), dataset__vintage=vintage,
            geoid__in=all_geoids
        )